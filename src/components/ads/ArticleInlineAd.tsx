...
export default function ArticleInlineAd({ ad }: ArticleInlineAdProps) {
  ...
  const alt = ad?.keterangan?.trim() || "Space iklan dalam artikel korankuid";
  ...
  // Responsive fallback banner (Navy gradient with red icon & CTA button)
  const placeholderCard = (
    <div className="w-full my-6 rounded-xl border border-[#28579e] bg-gradient-to-r from-[#052962] to-[#0b3d7e] p-3 sm:p-4 text-white shadow-2xs">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 rounded-lg border border-dashed border-[#6f91bf]/60 px-3 py-3 sm:px-5 sm:py-3.5">
        <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
          {/* Red icon */}
          <div className="w-10 h-10 sm:w-12 sm:h-12 shrink-0 rounded-lg bg-[#c91825] flex flex-col justify-center items-center shadow-xs">
          ...
          <div>
            <h4 className="text-sm sm:text-base font-extrabold tracking-wider uppercase font-serif">
              SPACE IKLAN
            </h4>
            <p className="text-[11px] sm:text-xs text-[#b8c7df]">
              Banner dalam artikel • 960 × 160
            </p>
          </div>
        </div>
